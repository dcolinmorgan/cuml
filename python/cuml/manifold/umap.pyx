...
class UMAP(UniversalBase,
    ...
    hash_input: bool, optional (default = True)
        ...
        feature has been made default for matters of reproducibility,
        but if not intending to use ``model.fit().transform()``, is 
        suboptimal in the due to the significant overhead in copying memory 
        to the host for computing the hash.
